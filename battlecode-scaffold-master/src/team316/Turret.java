...
public class Turret implements Player {
	...
	public void ttmCode(RobotController rc) throws GameActionException {
		// RobotInfo[] visibleEnemyArray =
		// rc.senseHostileRobots(rc.getLocation(), 1000000);
		Signal[] incomingSignals = rc.emptySignalQueue();
		// MapLocation[] enemyArray = combineThings(rc,
		// visibleEnemyArray,incomingSignals);
		Direction recommendedDirection = null;
		for (Signal s : incomingSignals) {
			MapLocation enemyLocation = getTurretEnemyMessage(rc, s);
			if (enemyLocation != null) {
				int distanceToEnemy = rc.getLocation()
						.distanceSquaredTo(enemyLocation);
			...
			rc.setIndicatorString(0, "I was recommended to move in direction:"
					+ RobotPlayer.directionToInt(recommendedDirection));
		}
		...
		rc.unpack();
		// TODO TTM should see if there are guards near by that it should escape
		// from.
	}
	...
	public void turretCode(RobotController rc) throws GameActionException {
		...
		for (Signal s : incomingSignals) {
			...
			if (enemyLocation != null && rc.canAttackLocation(enemyLocation)) {
				rc.setIndicatorString(0,
						"Discovered an enemy around using signal.");
				enemiesAround = true;
				if (rc.isWeaponReady()) {
					rc.setIndicatorString(0,
							"Attacking an enemy known using signal.");
		...
		RobotInfo[] visibleEnemyArray = rc.senseHostileRobots(rc.getLocation(),
				1000000);
		MapLocation[] enemyArray = combineThings(rc, visibleEnemyArray,
				incomingSignals);
	...
	private static MapLocation[] combineThings(RobotController rc,
			RobotInfo[] visibleEnemyArray, Signal[] incomingSignals) {
	...
	private static MapLocation getTurretEnemyMessage(RobotController rc,
			Signal s) {
		int[] message = s.getMessage();
		if (s.getTeam().equals(rc.getTeam()) && message != null
			...
			if (message[0] == RobotPlayer.MESSAGE_ENEMY) {
				return Encoding.decodeLocation(message[1]);
